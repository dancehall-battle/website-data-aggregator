...
module.exports = async (rankings) => {
  ...
  dancers.forEach(dancer => {
    getPostfix(dancer);
    dancer.ranks = {
      dancerCombined: getRankForDancer(rankings.dancerCombined.map, dancer.id),
      dancer2vs2: getRankForDancer(rankings.dancer2vs2.map, dancer.id),
      dancer1vs1: getRankForDancer(rankings.dancer1vs1.map, dancer.id)
    }
